...
# Simple class that can be pickled for test mocking
class _MockDoc:
...
class TestDateRulesPagination(IntegrationTestCase):
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.get_all")
    @patch("kreativ_notification.notification.rules_engine.frappe.get_meta")
    @patch("kreativ_notification.notification.rules_engine.nowdate")
    def test_evaluate_date_rules_paginates_over_500(self, mock_nowdate, mock_get_meta, mock_get_all):
        """More than 500 matching docs → processes all in 500-item chunks."""
        # Patch nowdate BEFORE it's called to avoid System Settings cache
        mock_nowdate.return_value = "2024-01-15"

        # Need to handle TWO get_all calls: 1) Notification Rules, 2) Document query
        rule_data = [
            _MockDoc(name="Test Date Rule", document_type="Test DocType", event="Days Before",
                     date_field="due_date", days_offset=0, enabled=1)
        ]

        call_count = [0]
        def mock_get_all_side_effect(*args, **kwargs):
            # First call is for Notification Rules (doctype="Notification Rule")
            # Second+ calls are for the document type query
            if args and args[0] == "Notification Rule":
                return rule_data
            call_count[0] += 1
            if call_count[0] == 1:
        ...
        # Mock frappe.get_doc to return a simple pickleable object
        with patch("kreativ_notification.notification.rules_engine.frappe.get_doc") as mock_get_doc:
            mock_get_doc.return_value = _MockDoc(name="test", doctype="Test DocType")
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.get_meta")
    @patch("kreativ_notification.notification.rules_engine.nowdate")
    def test_evaluate_date_rules_stops_at_empty_page(self, mock_nowdate, mock_get_meta, mock_get_all):
        """Stops when a page returns fewer than 500 docs."""
        # Patch nowdate BEFORE it's called to avoid System Settings cache
        mock_nowdate.return_value = "2024-01-15"

        rule_data = [
            _MockDoc(name="Test Date Rule 2", document_type="Test DocType", event="Days Before",
                     date_field="due_date", days_offset=0, enabled=1)
        ]
...
class TestIdempotencyKeys(IntegrationTestCase):
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.get_cached_doc")
    def test_value_change_idempotency_includes_new_value(self, mock_get_cached_doc):
        ...
        def get_cached_doc_side_effect(dt, name):
            if dt == "Notification Rule":
                return rule_mock
            return template_mock
        mock_get_cached_doc.side_effect = get_cached_doc_side_effect
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.get_cached_doc")
    def test_value_change_skips_if_field_unchanged(self, mock_get_cached_doc):
        """Value Change rule skips when watched field hasn't changed."""
        rule_mock = Mock()
        rule_mock.name = "VC Test Rule 2"
        rule_mock.event = "Value Change"
        rule_mock.value_changed_field = "status"
        rule_mock.applies_to.return_value = True
        mock_get_cached_doc.return_value = rule_mock
...
class TestRuleProcessing(IntegrationTestCase):
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.get_cached_doc")
    def test_applies_to_filters_document(self, mock_get_cached_doc):
        """applies_to() filters by condition."""
        rule_mock = Mock()
        rule_mock.name = "Filter Rule"
        rule_mock.event = "New"
        rule_mock.applies_to.return_value = False  # Condition fails
        rule_mock.message_template = "MT-001"
        rule_mock.channel = "WhatsApp - OpenWA"
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.get_cached_doc")
    def test_disabled_template_skips_dispatch(self, mock_get_cached_doc):
        """Disabled message template skips dispatch."""
        rule_mock = Mock()
        rule_mock.name = "Disabled Template Rule"
        rule_mock.event = "New"
        rule_mock.applies_to.return_value = True
        rule_mock.message_template = "MT-Disabled"
        rule_mock.channel = "WhatsApp - OpenWA"
        rule_mock.resolve_recipients.return_value = ["919999999999@c.us"]
        rule_mock.get_recipient_language.return_value = "en"
        ...
        mock_get_cached_doc.side_effect = lambda dt, name: rule_mock if dt == "Notification Rule" else template_mock
...
class TestShiftHoursHelper(IntegrationTestCase):
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.db.exists")
    @patch("kreativ_notification.notification.rules_engine.frappe.db.get_value")
    def test_calculates_from_attendance_shift(self, mock_get_value, mock_exists):
        """Uses KG Employee Attendance Shift when available."""
        mock_exists.return_value = True
        mock_get_value.return_value = "09:00"
    ...
    @patch("kreativ_notification.notification.rules_engine.frappe.db.exists")
    @patch("kreativ_notification.notification.rules_engine.frappe.db.get_value")
    def test_falls_back_to_checkin_if_no_shift_doctype(self, mock_get_value, mock_exists):
        """Falls back to Employee Checkin when shift doctype missing."""
        mock_exists.return_value = False

        # First call is for Employee Checkin (the fallback)
        mock_get_value.return_value = "2024-01-15 09:00:00"

        result = _get_shift_hours_for_out("EMP-002", "2024-01-15 18:00:00")
        # Should calculate from check-in time
